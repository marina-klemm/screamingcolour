...
# Define sentiment emoji mapping
sentiment_emoji_map <- list(
  "😠" = c("angry", "resentful", "vengeful", "defiant"),
  "😊" = c("happy", "playful", "joyful", "grateful", "admired", "excited", "peaceful", "warm"),
  "😔" = c("regretful", "hopeless", "vulnerable", "concerned", "anxious", "nostalgic", "melancholic", "frustrated", "sad", "desperate"),
  "💪" = c("encouraging", "empowering", "free", "optimistic", "empathetic", "hopeful", "compassion", "brave", "resolute", "passionate"),
  "😏" = c("tempted", "reflective", "conflicted", "pleading", "accepting", "reminiscent"),
  "💔" = c("heartbroken", "lovesick", "grieving", "longing", "lonely")
)

# Create reverse mapping (sentiment to emoji)
sentiment_to_emoji <- list()
for (emoji in names(sentiment_emoji_map)) {
  for (sentiment in sentiment_emoji_map[[emoji]]) {
    sentiment_to_emoji[[sentiment]] <- emoji
  }
}
