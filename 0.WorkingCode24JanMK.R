...
pathToDressColours <- "C:/Users/mklemm/OneDrive - AUCKLAND MUSEUM/Documents/Personal/FunAnalysis/DressesColours/Cropped/"
...
ggplot(pianoSongsDataWithImages, aes(x = reorder(DressName, n), y = n, fill = DressName)) +
  geom_bar(stat = "identity") +
  geom_image(aes(image = imagePath), size = 0.1, by = "width") + # Adjust `size` as needed
  geom_text(aes(label = paste0(round(percentage, 1), "%")), 
            vjust = -0.5, size = 3) +
  scale_fill_manual(values = colorPaletteDresses) +
  theme_minimal() +
...
ggplot(pianoSongsDataWithImages, aes(x=reorder(DressName, n), y= n, fill = DressName)) +
  geom_col() +
  geom_image(aes(image=imagePath, y=n), size = .1)

## Trying to fit the images inside the bars

pianoSongsDataWithImages <- pianoSongsData %>%
  count(DressName) %>%
  mutate(
    percentage = n / sum(n) * 100,
    imagePath = case_when(
      ...
      DressName == "Grapefruit" ~ paste0(pathToDressColours,"Grapefruit.jpg"),
      DressName == "Popsicle" ~ paste0(pathToDressColours,"Popsicle.jpg"),
      TRUE ~ paste0(pathToDressColours, "Default.jpg")
    ),
    value1 = 0.3 * n  # Position images at half the height of the bars
  )

colnames(pianoSongsDataWithImages)

# Creating the plot
ggplot(pianoSongsDataWithImages, aes(x = reorder(DressName, n), y = n)) + 
  geom_col(fill = "transparent",  position = position_dodge(width = 10)) +  
  geom_image(aes(image = imagePath, y = value1), size = 0.2) +  # Images placed at half the bar height
  geom_text(
    aes(label = paste0(round(percentage, 1), "%")), 
    vjust = -2, size = 5
  ) +
...
ggplot(pianoSongsDataWithImages, aes(x = reorder(DressName, n), y = n)) + 
  geom_col(fill = "transparent", position = position_dodge(width = 1)) +  
  geom_image(aes(image = imagePath, y = value1), size = 0.2) +  
  geom_text(
    aes(label = paste0(round(percentage, 1), "%")), 
    vjust = -2, size = 5
  ) +
...
unique(taylorSentimentsBing$sentiment)
...
# Relating taylorSentimentsBing and taylorSentimentsMetadataBing

head(taylorSentimentsBing$sentiment)
head(taylorSentimentsMetadataBing$sentiment)

# Combine sentiment data
taylorSentimentsBing_df <- data.frame(
  dataset = "taylorSentimentsBing",
  sentiment = taylorSentimentsBing$sentiment
)

taylorSentimentsMetadataBing_df <- data.frame(
  dataset = "taylorSentimentsMetadataBing", 
  sentiment = taylorSentimentsMetadataBing$sentiment
)

combined_sentiments <- rbind(
  taylorSentimentsBing_df, 
  taylorSentimentsMetadataBing_df
)

# Create plot
ggplot(combined_sentiments, aes(x = sentiment, fill = dataset)) +
...
## What about an actual correlation plot?

# Aggregate sentiment scores for each song
metadataSentimentScores <- taylorSentimentsMetadataBing %>%
  group_by(track_number, album_name, track_name) %>%
  summarize(metadata_sentiment_score = sum(sentiment, na.rm = TRUE))

lyricsSentimentScores <- taylorSentimentsBing %>%
  group_by(track_number, album_name, track_name) %>%
  summarize(lyrics_sentiment_score = sum(sentiment, na.rm = TRUE))

# Combine the aggregated scores into one dataset
combinedSentiments <- metadataSentimentScores %>%
  inner_join(lyricsSentimentScores, by = c("track_number", "album_name", "song_title"))
...
outliers <- combinedSentiments %>%
  filter(difference > 1.5) # Adjust threshold as needed
...
# Visualization
ggplot(combinedSentiments, aes(metadata_sentiment_score, lyrics_sentiment_score, label = song_title)) +
  geom_point(aes(color = album_name), size = 3) +
  geom_text_repel(data = outliers, aes(label = song_title), size = 3) +
  labs(
    title = "Correlation Between Sentiment Scores",
    x = "Metadata Sentiment Score",
